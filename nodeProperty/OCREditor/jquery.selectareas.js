...
(function ($) {
    ...
    $.imageArea = function (parent, id) {
        ...
        var options = parent.options,
            ...
            updateSelection = function () {
                ...
                //console.log("width:" + area.width + "," + "height:" + area.height + ",left:" + area.x + ",top:" + area.y);
                $outline.css({
                    cursor: "default",
                    width: area.width,
                    height: area.height,
                ...
                //console.log("SElecttion CSS : width:" + area.width + "," + "height:" + area.height + ",left:" + area.x + ",top:" + area.y);
            ...
            updateBtLabel = function (visible) {
                //console.log(area.id);
              //  console.log(id);
               
                if ($btLabel) {                   
                        $btLabel.css({
                            display: visible ? "block" : "block", // Changed By Rashad
                            left: area.x - 15,
                            top: area.y - $btLabel.outerHeight() + 15,
                            "z-index": area.z + 1
                        });
                    //$("#txt" + area.id).val(area.indx);
        ...
        /*
        * Updated By Rashad
        * Change Border color for selected area
        *  
        * 
        * 
        */
        ...
        if (area.id.length>0){
            $outline = $("<div class=\"select-areas-outline\" />") // Class For Black border area
            .css({
            ...
            $outline = $("<div class=\"select-areas-outline-load\" />") // Class For coloured border area
            .css({
                // opacity : options.outlineOpacity,
                position: "absolute"
            })
        ...
        if (options.allowLabel) {
            var bindToLabel = function ($obj) {
                return $obj;
               
            };
            var txtId = "txt" + area.id;
            $btLabel = bindToLabel($("<div class=\"label-area\" />"))
                .append(bindToLabel($("<input class=\"select-areas-label-area\" id=" + txtId + "  type=\"text\" />")))
                .insertAfter($selection);
        }
        if (options.allowMove) {
    ...
    $.imageSelectAreas.prototype.init = function (object, customOptions) {
        ...
        if (!this.options.allowEdit) {
            this.options.allowSelect = this.options.allowMove = this.options.allowResize = this.options.allowDelete = false; this.options.allowLabel;
        }
    ...
    $.imageSelectAreas.prototype.add = function (options) {
        ...
        this._refresh();
        if (!this.options.allowSelect && !this.options.allowMove && !this.options.allowResize && !this.options.allowDelete && !this.options.allowLabel) {
            this.blurAll();
        }
    };
