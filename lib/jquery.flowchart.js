$(function () {
    ...
    $.widget("flowchart.flowchart", {
        ...
        _drawLink: function (linkId) {
            ...
            var fromSmallConnector = fromOperator.internal.els.connectorArrows[fromConnectorId][fromSubConnector];
            var toSmallConnector = toOperator.internal.els.connectorArrows[toConnectorId][toSubConnector];
            ...
            shape_path.setAttribute("stroke-width", "3");
        ...
        _getOperatorFullElement: function (operatorData) {
            ...
            var $operator_title = $('<div class="flowchart-operator-connector-label"></div>');
            //var $operator_image = $('<div class="flowchart-operator-inputs-outputs-icon"><image class="palette_icon" src="./icons/textanalytics.png"></image></div>');
            var $operator_image = $('<div class="flowchart-operator-inputs-outputs-icon"><image class="palette_icon" src="./icons/node/'+infos.id+'.png"></image></div>');
            $operator_image.appendTo($operator_inputs_outputs);

            $operator_title.html(infos.title);
            $operator_title.appendTo($operator_inputs_outputs);
        ...
        addOperator: function (operatorData) {
            ...
            this.createOperator(operatorData.operator_id, operatorData);
        ...
        createOperator: function (operatorId, operatorData) {
            ...
            if (this.options.canUserMoveOperators) {
                ...
                fullElement.operator.draggable({
                    ...
                    handle: '.flowchart-operator-connector-label',
                    start: function (e, ui) {
            ...
            for (var keyI in keys) {
                if (keys.hasOwnProperty(keyI)) {
                    ...
                    for (var objId in data[key]) {
                        if (data.operators[objId] != undefined) {
                            var flow = {};
                            flow["id"] = objId;
                            flow["type"] = data.operators[objId]["properties"].id;
                            flow["wires"] = [];
                            $('form#' + objId + ' :input').each(function () {
                                if(this.id!=""){
                                  flow[this.id] = ($(this).val()==null) ? "" : $(this).val();
                                }
                            });
                            // $('form#' + objId + ' select option:selected').each(function () {
                            //     flow[this.parentNode.id] = $(this).text();
                            // });
                            data.operators[objId]["properties"].flow = flow;
                        }
                        if (data.links[objId] != undefined) {
                            var fromOperator = data.links[objId].fromOperator;
                            var toOperator = data.links[objId].toOperator;
                            var wire=[];
                            wire.push(toOperator);
                            data.operators[fromOperator]["properties"].flow.wires.push(wire);
                        }
                        if (data.operators[objId] != undefined)
        ...
        getPropertyData: function (formId) {
            var flow = {};
            $("form#" + formId + ":input").each(function () {
                flow[this.name] = $(this).val();
            });
            return flow;
        },
